...
#Start loop
qreg_list <- foreach(lipid_var = lipid_vars,
                     ...
                     .packages = packs) %do% {
                       ...
                       m <- mqgam(data = x, #Data
                                  ...
                                  argGam = list(weights = x$weights/mean(x$weights, na.rm = TRUE)) #Apply survey Weights
                                  )
                       ...
                       #Graph it
                       source("Discordance Figures.R", local = TRUE)
...
names(qreg_list) <- lipid_vars

#Save the model data needed for the Shiny app
save(file = "Calculator pre-requisities.RData", qreg_list)